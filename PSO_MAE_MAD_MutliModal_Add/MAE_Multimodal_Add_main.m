%
%设施优化布局：Max_Equality Multimodal Add Resource with PSO
%
clear
global AC_c;
global AC_p;
...
dc0=90; %服务半径参数D，驾车模式
dp0=90; %服务半径参数D，公交模式
TNS = 4622;  %新增设施规模总和；根据深圳卫生事业十四五规划，提升23.2%。
...
range=zeros(num,2);
for i=1:num
   range(i,:)=[0,200];   %设施规模的上限下限;相对大小起作用
end
Max_V = 0.2*(range(:,2)-range(:,1)); %velocity
Pdef=[100 4000 48 2 2 0.9 0.4 1500 1e-15 400 NaN 0 0];
tic
rlt = pso_Trelea_vectorized('MAE_Multimodal_Add_fun',num,Max_V,range,0,Pdef); %pso;num+1行，最后一行为目标函数值
toc

%根据最优解，计算优化可达性
optNS = rlt(1:m); %优化新增规模
optS = optNS + AS;
for j=1:m
   for i=1:n
      bestB_c(i,j)=optS(j)*AC_c(i,j)*h_c(i)/(Total_c(j)+Total_p(j)); %需求点i从设施点j分配到的床位数
      bestB_p(i,j)=optS(j)*AC_p(i,j)*h_p(i)/(Total_c(j)+Total_p(j)); %需求点i从设施点j分配到的床位数
   end
end
